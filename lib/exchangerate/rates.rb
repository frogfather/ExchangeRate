...
@DataFromFile = ""
  ...
  def Rates.Request(url = @defaultUrl)    
    done = false 
    loopCount = 0
    #try three times to get the data
    while !done do      
      Connection::Sendrequest(@defaultUrl)
  ...
  def Rates.GetConversion(date,base,counter)
    ...
    return 0 if (baseRate == "" || counterRate == "")
  ...
  def Rates.GetRate(date, currency)
    ...
    return "" if (!@fxData.has_key?(dateKey))
